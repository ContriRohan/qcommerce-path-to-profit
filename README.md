...
## Real-geography validation (OpenStreetMap)

The results above calibrate to real published FY26 financials. This step goes further
and runs the twin on the **real spatial structure of real cities**. The demand
surface is built from the real location of ~10,800 shops and amenities in Bengaluru
and ~520 in Lucknow (OpenStreetMap via Overpass), and the intraday arrival shape is
fitted to real order timestamps (25,900 orders, UCI Online Retail). A dark store is
placed at the densest real cell, and its order volume and drop distances fall out of
the gravity catchment over the real grid - nothing hand-set.

Run it with `make real`:

| city (real geography) | orders/day | mean drop | contribution/order |
| --- | --- | --- | --- |
| Bengaluru (metro) | **1,247** | 0.99 km | **-Rs 2.11** |
| Lucknow (tier-2) | **350** | 0.39 km | **-Rs 38.76** |

Two things worth noting. The metro store on real Bengaluru geography lands at
**-Rs 2.11/order**, independently close to the real Blinkit FY26 figure of -Rs 3.02
that the model was never fit to at the city level. And the tier-2 store is
**Rs 36.6/order worse**, driven by **3.6x lower order density** - the same "density is
the binding constraint" conclusion, now falling out of real geography rather than a
synthetic grid. Lucknow's monocentric core even gives it *shorter* drops (0.39 km),
yet it still bleeds, because there simply are not enough orders to spread the fixed
crew over.

![Real-geography comparison](reports/real_city.png)
...
python -m qcom.cli real                                           # real OSM city geography
```

`make real` (or the `real` command) runs on committed real artifacts and needs no
network. To refetch them from source, see [data/geo/README.md](data/geo/README.md)
(OpenStreetMap) and [data/raw/README.md](data/raw/README.md) (UCI Online Retail).
